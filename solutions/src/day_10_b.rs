...
//       Pipe::Vertical => '|',
//       Pipe::Horizontal => '-',
//       Pipe::Corner(Direction::North, Direction::East) => 'L',
//       Pipe::Corner(Direction::North, Direction::West) => 'J',
//       Pipe::Corner(Direction::South, Direction::West) => '7',
//       Pipe::Corner(Direction::South, Direction::East) => 'F',
//       Pipe::Ground => '.',
//       Pipe::Starting => 'S',
//       _ => panic!("Error"),
//     }
...
fn find_all_captured_pieces(contents: String) -> u32 {
  ...
  return just_iterate_again(&map, width, height, &path);
...
fn just_iterate_again(map: &HashMap<(usize, usize), Pipe>, width: usize, height: usize, path: &HashSet<(usize, usize)>) -> u32 {
  let mut total: u16 = 0;
  for j in 0..height {
    let mut primed: Option<Direction> = None;
    let mut crossed: u8 = 0;
    for i in 0..width {
      let pos = (i, j);
      let in_path = path.contains(&pos);
      if !in_path && crossed % 2 == 1 {
        total += 1;
      } else if in_path {
        match (map.get(&pos), primed) {
          (Some(Pipe::Vertical), _) => crossed += 1,
          (Some(Pipe::Corner(prime_dir, Direction::East)), None) => primed = Some(*prime_dir),
          (Some(Pipe::Corner(Direction::North, Direction::West)), Some(Direction::South)) |
          (Some(Pipe::Corner(Direction::South, Direction::West)), Some(Direction::North)) => {
            primed = None;
            crossed += 1;
          },
          (Some(Pipe::Corner(_, _)), Some(_)) => primed = None,
          (_, _) => {}
        }
      }
    }
